	...
	public static void registerCustomRecipes()
	{
		...
		for(int i = 0; i < customRecipes.length; i++)
		{
			...
			if((i & 1) == 0 || i == 0)
			{
				...
				if(core == null)
				{
					FMLLog.log(Level.ERROR, new IllegalArgumentException("The name for core item: " + customRecipes[i] + "is invalid. Please double check"), "", "");
				}
				...
				if(i != customRecipes.length-1)
				{
					...
					if(output == null)
					{
						FMLLog.log(Level.ERROR, new IllegalArgumentException("The name for output item: " + customRecipes[i+1] + "is invalid. Please double check"), "", "");
					}

					if(output != null && core != null)
					{
						FIHelper.setOutputCoreCustom(i+1, output, core);
						GameRegistry.addRecipe(new RecipeWorldSpecificCustom(output, i+1));
					}
				}
